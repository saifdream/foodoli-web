...
const Confirmed = memo(
    () => {
        ...
        const {confirmedReservationList, totalConfirmedPages, isLoading} = useSelector(state => state.restaurants);
        const [page, setPage] = useState(1);

        useEffect(() => {
            console.log("Fire")
            dispatch(getReservation('Confirmed', user?.rest_id, page));
        }, []);

        console.log("Before Rendering")

        if(!confirmedReservationList) {
            ...
            <Box mt={2}>
                ...
                <TableContainer component={Paper}>
                    <Table className={classes.table} aria-label="simple table">
                        ...
                        <TableBody>
                            {
                                confirmedReservationList.length === 0 && <TableCell align="center" colSpan={10}>No reservation found</TableCell>
                            }
                            {
                                confirmedReservationList.map((row, index) => (
                                    <TableRow key={row.id}>
                        ...
                        </TableBody>
                        {
                            totalConfirmedPages > 1 && (
                                <TableFooter>
                                    <TableRow>
                                        <TableCell colSpan={8}>Total Pages: {totalConfirmedPages} Current Page: {page}</TableCell>
                                        <TableCell align="right">
                                            <IconButton onClick={()=> {
                                                if(!isLoading && page > 1) {
                                                    setPage(page - 1);
                                                    dispatch(getReservation('Confirmed', user?.rest_id, page + 1));
                                                }
                                            }}>
                                                <ArrowBack/>
                                            </IconButton>
                                        ...
                                        <TableCell align="left">
                                            <IconButton onClick={()=> {
                                                if(!isLoading && totalConfirmedPages > page) {
                                                    setPage(page + 1);
                                                    dispatch(getReservation('Confirmed', user?.rest_id, page + 1));
                                                }
                                            }}>
